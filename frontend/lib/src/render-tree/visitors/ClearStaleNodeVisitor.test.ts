...
describe("ClearStaleNodeVisitor", () => {
  ...
  describe("visitTransientNode", () => {
    it("returns node unchanged when in fragment run", () => {
      const t = new TransientNode("run", text("anchor"), [text("t1")], 1)
      const visitor = new ClearStaleNodeVisitor("run", ["frag1"]) // fragment run
